...
await page.goto('http://localhost:5174/#/components', { waitUntil: 'networkidle', timeout: 15000 });
...
const gridContent = await page.$$eval('.card-grid', grids => 
  grids.map(g => ({
    ...
    hasMkCard: Array.from(g.children).some(c => c.classList.contains('mk-card'))
  }))
);
console.log('Card grids:', JSON.stringify(gridContent, null, 2));

// Check for any mk-card--shadow-hover
const hoverCards = await page.$$eval('.mk-card--shadow-hover', els => els.length);
...
const firstCardHtml = await page.$eval('.mk-card', el => el.outerHTML.slice(0, 500)).catch(() => 'no card found');
