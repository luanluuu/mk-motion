...
export interface FormContext {
  ...
  updateFieldValue: (prop: string, value: any) => void
}

export const formContextKey: InjectionKey<FormContext> = Symbol('MkFormContext')

export function validateValue(value: any, rules: FormRule[], label = ''): string | undefined {
  for (const rule of rules) {
    ...
    if (rule.validator) {
      ...
      if (result !== true) return typeof result === 'string' ? result : rule.message || `${label || '该项'}校验失败`
    }
  }
  return undefined
}
