...
onMounted(() => {
  if (!btnRef.value) return
  const springOpts = props.spring === true ? undefined : props.spring || undefined
  const hoverCtrl = springHover(btnRef.value, { scale: 1.03, y: -1, shadow: true, spring: springOpts })
  const pressCtrl = springPress(btnRef.value, { scale: 0.97, spring: springOpts })
  motionCtrl = { destroy: () => { hoverCtrl.destroy(); pressCtrl.destroy() } }
})
